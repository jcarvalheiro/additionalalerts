...
class PluginAdditionalalertsTicketNoCategory extends CommonDBTM {
   static $rightname = "plugin_additionalalerts";

   static function getTypeName($nb = 0) {
      return _n('Ticket with no category', 'Tickets with no category', $nb, 'additionalalerts');
   }
   ...
   static function cronInfo($name) {
      switch ($name) {
         case 'AdditionalalertsTicketNoCategory':
            return [
               'description' => PluginAdditionalalertsTicketNoCategory::getTypeName(2)
            ];
      }
      return [];
   }
