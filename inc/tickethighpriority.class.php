...
/**
 * Class PluginAdditionalalertsTicketHighPriority
 */
class PluginAdditionalalertsTicketHighPriority extends CommonDBTM {

   static $rightname = "plugin_additionalalerts";

   static function getTypeName($nb = 0) {
      return _n('High priority ticket not processed', 'High priority tickets not processed', $nb, 'additionalalerts');
   }
   ...
   static function cronInfo($name) {
      switch ($name) {
         case 'AdditionalalertsTicketHighPriority':
            return [
               'description' => PluginAdditionalalertsTicketHighPriority::getTypeName(2)
            ];
            break;
      }
      return [];
