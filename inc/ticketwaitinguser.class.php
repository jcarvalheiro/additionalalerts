...
/**
 * Class PluginAdditionalalertsTicketWaitingUser
 */
class PluginAdditionalalertsTicketWaitingUser extends CommonDBTM {

   static $rightname = "plugin_additionalalerts";

   static function getTypeName($nb = 0) {
      return _n('Ticket waiting for user response', 'Tickets waiting for user response', $nb, 'additionalalerts');
   }
   ...
   static function cronInfo($name) {
      switch ($name) {
         case 'AdditionalalertsTicketWaitingUser':
            return [
               'description' => PluginAdditionalalertsTicketWaitingUser::getTypeName(2)
            ];
            break;
      }
      return [];
